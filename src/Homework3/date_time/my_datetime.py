from my_time.my_time import Time
from my_date.my_date import Date
...
class DateTime:
    ...
    def __add__(self, other: "date_time"):
    ...
    def __sub__(self, other: "date_time"):
    ...
    def sub_day(self, d: int):
        if self.__date.day - d <= 0:
            ...
            while delta <= 0:
                c_days = self.__date.day_in_mouth(self.__date.mount-1)
                if delta > c_days:
                    delta += c_days
                    self.sub_month(1)
                else:
